...
extension AnnotationOf on Annotation {
  ElementAnnotation? annotationOfType(TypeChecker type, {required bool exact}) {
    ...
    final annotationElement = this.element;
    if (annotationElement == null || elementAnnotation == null) return null;
    if (annotationElement is! ExecutableElement) return null;

    if ((exact && !type.isExactlyType(annotationElement.returnType)) ||
        (!exact && !type.isAssignableFromType(annotationElement.returnType))) {
      return null;
    }
